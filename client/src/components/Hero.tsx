...
export default function Hero({ onContactOpen: _onContactOpen }: HeroProps) {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && charIndex < current.length)
      ...
      t = setTimeout(() => { setIsDeleting(false); setRoleIndex(r => (r + 1) % ROLES.length); }, 0);
  ...
  return (
    <section ref={containerRef} id="home" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="absolute right-0 top-0 w-[65%] h-full pointer-events-none"
        style={{ background: "radial-gradient(ellipse 65% 75% at 78% 42%, rgba(168,85,247,0.15) 0%, transparent 70%)" }}
      />
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-4">
            ...
            <div className="flex-1 max-w-lg z-20">
              ...
              >
                ...
                />
                <span className="text-[11px] font-mono tracking-[0.22em] uppercase" style={{ color: ACCENT }}>
                ...
                {titleLines.map((word, i) => (
                  <div key={i} className="overflow-hidden leading-[1.05]">
                    <motion.h1
                      ...
                      style={{ color: i === 1 ? ACCENT : "var(--text-primary, inherit)" }}
                      initial={{ y: "110%" }} animate={{ y: "0%" }}
                      transition={{ duration: 0.9, delay: 0.2 + i * 0.13, ease: [0.16, 1, 0.3, 1] }}
                    >
                      {word}
                    </motion.h1>
                  </div>
              ...
              >
                <span className="font-mono text-sm tracking-wider" style={{ color: "var(--text-secondary, #888)" }}>
                  {displayText}
                </span>
                <motion.span
              ...
              >
                <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" data-testid="button-view-cv"
                ...
                <button onClick={() => scrollTo("#projects")} data-testid="button-see-work"
            ...
            >
              ...
              >
                ...
                {[0, 2, 4].map((i) => {
                  ...
                  return (
                    <line key={i}
                      x1={C + r * Math.cos(a1)} y1={C + r * Math.sin(a1)}
                      x2={C + r * Math.cos(a2)} y2={C + r * Math.sin(a2)}
                      stroke={ACCENT}
                      strokeWidth="0.6"
                      strokeOpacity="0.12"
                    />
                ...
                {[0, 120, 240].map((deg, i) => (
                  <motion.line key={`scan${i}`}
                    ...
                    strokeOpacity="0.5"
                    animate={{ rotate: 360 }}
                    transition={{ duration: 10 + i * 4, repeat: Infinity, ease: "linear", delay: i * 1.2 }}
                    style={{ transformOrigin: `${C}px ${C}px` }}
                  />
                ))}

                {/* Orbiting dots */}
                <motion.circle r="5" fill={ACCENT_BRIGHT}
                  cx={C + 190} cy={C}
                ...
                {[
                  ...
                  <path key={`b${i}`} d={b.d}
                    ...
                    fill="none" stroke={ACCENT_BRIGHT}
                ...
                {/* HUD labels */}
                <text x={C + 152} y={C - 152} fill={ACCENT_BRIGHT} fontSize="9"
                  fontFamily="monospace" opacity="0.85" textAnchor="middle" fontWeight="600">
                  DEV
                </text>
                <text x={C - 152} y={C + 158} fill={ACCENT_BRIGHT} fontSize="9"
                  fontFamily="monospace" opacity="0.85" textAnchor="middle" fontWeight="600">
                  UI/UX
                </text>
                <text x={C + 152} y={C + 158} fill={ACCENT} fontSize="8"
                  fontFamily="monospace" opacity="0.6" textAnchor="middle">
                  2025
                </text>
              </svg>
              ...
              <motion.div
                ...
                style={{
                  width: "260px", height: "260px", zIndex: 8,
                  top: "50%", left: "50%",
              ...
              <motion.div
                className="absolute rounded-full pointer-events-none"
                style={{
                  width: "244px", height: "244px", zIndex: 9,
                  top: "50%", left: "50%",
                  transform: "translate(-50%, -50%)",
                }}
                animate={{
              ...
              >
                <div className="absolute inset-0 z-10 pointer-events-none rounded-full"
                  style={{ background: "linear-gradient(145deg, rgba(255,255,255,0.06) 0%, transparent 55%, rgba(168,85,247,0.10) 100%)" }}
                />
                <img src="/Rakhi-portfolio.jpg" alt="Rakhi"
      ...
      >
        <motion.button onClick={() => scrollTo("#about")} data-testid="button-scroll-down"
          animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
          className="flex flex-col items-center gap-1.5 transition-colors font-mono text-[10px] tracking-widest uppercase"
